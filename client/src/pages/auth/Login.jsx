...
export default function Login() {
  ...
  const isGoogleConfigured = googleClientId && !googleClientId.includes('your-google') && googleClientId !== 'placeholder' && googleClientId.length > 20;
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-green-50 flex items-center justify-center py-12 px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8 border border-gray-100">
          ...
          {/* Google Login - matches existing rounded-xl, border, shadow style - ALWAYS VISIBLE */}
          <div className="w-full">
            {isGoogleConfigured ? (
              ...
              <button
                type="button"
                onClick={() => toast.error('Google Client ID not configured. Add VITE_GOOGLE_CLIENT_ID to client/.env and restart. See client/.env.example')}
                className="w-full py-3 border border-gray-200 rounded-xl flex items-center justify-center gap-3 bg-white text-gray-700 font-semibold hover:bg-gray-50 hover:border-gray-300 transition-all shadow-sm"
              >
          ...
          <p className="text-xs text-gray-400 text-center mt-3">
            {isGoogleConfigured ? 'Secure authentication via Google' : 'Demo: configure Google OAuth to enable'}
          </p>
